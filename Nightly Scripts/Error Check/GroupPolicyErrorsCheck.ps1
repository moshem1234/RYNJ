...
ForEach ($Server in $PCs) {
	...
	If (Test-Connection -ComputerName $Server -Quiet -Count 1 -ErrorAction SilentlyContinue) {
		...
		$Var += Get-WinEvent -ComputerName $Server -LogName "Microsoft-Windows-GroupPolicy/Operational" -FilterXPath "*[System[(Level=0 or Level=1 or Level=3 or Level=5)]]" -ErrorAction SilentlyContinue | Select-Object @{name='ComputerName'; expression={"$Server"}},TimeCreated,Level,Message | Where-Object -FilterScript {($_.TimeCreated -GT $Date) -and ($_.Message -NOTLIKE "*This machine is configured to retrieve Group Policy files from a file share in an insecure way.*")} | Format-Table -AutoSize -Wrap | Out-String
	}
	$count += 1
}
