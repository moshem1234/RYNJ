...
ForEach ($Server in $PCs) {
	Write-Progress -Activity "Checking Group Policy Logs" -Status $Server -PercentComplete (($count / $PCs.Count) * 100)
	If (Test-Connection -ComputerName $Server -Quiet -Count 1 -ErrorAction SilentlyContinue) {
		...
		$Var += Get-WinEvent -ComputerName $Server -LogName "Microsoft-Windows-GroupPolicy/Operational" -FilterXPath "*[System[(Level=0 or Level=1 or Level=3 or Level=5)]]" -ErrorAction SilentlyContinue | Select-Object @{name='ComputerName'; expression={"$Server"}},TimeCreated,Message | Where-Object -FilterScript {($_.TimeCreated -GT $Date) -and ($_.Message -NOTLIKE "*This machine is configured to retrieve Group Policy files from a file share in an insecure way.*")} | Format-Table -AutoSize -Wrap | Out-String
	}
	$count += 1
}
